...
const annotationPanel = createAnnotationPanel(shell.panelBindings);
...
function syncJourneyClasses() {
  ...
  document.body.classList.toggle("final-star-lit", journeyState.finalStep >= 2);
}
...
function updateMissionForStage() {
  if (activeStage === 1 && !journeyState.rocketCaught) {
    showMission(journeyState.rocketPrompted ? "Нажми на ракету еще раз, когда она рядом с Землей." : "Ракета кружит рядом с Землей. Нажми на нее, чтобы поймать момент.");
    return;
  }
  ...
  if (activeStage === 2) {
    ...
    showMission(`Артефакты двигателя: ${count}/${total}. Нажимай на планеты, чтобы собрать детали.`);
    return;
  }

  if (activeStage === 6 && !journeyState.webComplete) {
    showMission(journeyState.webQuestStarted ? "Держись светящейся нити и заверши маршрут." : "Проложи путь корабля по космической нити.");
    return;
  }

  if (activeStage === 7 && journeyState.finalStep < 2) {
    showMission(journeyState.finalStep === 0 ? "Нажми кнопку финала, чтобы продолжить рассказ." : "Зажги свою звезду в темноте.");
    return;
  }
...
function collectSolarArtifact(data) {
  ...
  if (journeyState.artifactIds.size >= journeyState.artifactTotal) {
    ...
    showMission("Все детали двигателя собраны. Можно лететь дальше.");
    return;
  }

  showMission(`Артефакты двигателя: ${journeyState.artifactIds.size}/${journeyState.artifactTotal}. ${data.title} отдала свою деталь.`);
}
...
function runWebPath() {
  ...
  if (!journeyState.webQuestStarted) {
    ...
    webRunner.textContent = "Довести корабль";
    setNarration("universeQuest", { force: true });
    showMission("Корабль держится на светящейся нити. Нажми еще раз, чтобы завершить маршрут.");
    syncJourneyClasses();
    return;
  }
  ...
  webRunner.textContent = "Маршрут пройден";
  setNarration("universeComplete", { force: true });
  showMission("Маршрут по космической нити пройден. Листай к неизвестному.");
  syncJourneyClasses();
}

function slipFromWebPath() {
  if (!journeyState.webQuestStarted || journeyState.webComplete || activeStage !== 6) {
    return;
  }

  setNarration("universeFall", { force: true });
  showMission("Сорвались в пустоту. Вернись на нить кнопкой маршрута.");
}

function createPersonalStar() {
  const star = document.createElement("span");
  star.style.left = `${18 + Math.random() * 64}%`;
  star.style.top = `${16 + Math.random() * 52}%`;
  star.style.setProperty("--twinkle-delay", `${Math.random() * 1.8}s`);
...
function advanceFinale() {
  ...
  if (journeyState.finalStep === 0) {
    ...
    starMaker.textContent = "Оставить след";
    setNarration("finalTwo", { force: true });
    showMission("Теперь можно оставить свой след в темноте.");
    syncJourneyClasses();
    return;
  }
  ...
  if (journeyState.finalStep === 1) {
    ...
    showMission("Твоя звезда теперь светит на краю карты.");
    syncJourneyClasses();
  }
}
...
function onClick(event) {
  ...
  if (event.target.closest(".narration-panel, .rocket-catcher, .web-runner, .star-maker, .cinematic-intro, .object-panel, .scale-rail, .distance-summary, .topbar, .space-label, #locateButton")) {
    return;
  }
